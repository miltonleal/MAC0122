...
def CLASSIFICA(TAB, ordem):
    ...
    def cmp_to_key(mycmp):
        ...
        class K:
            ...
            def __lt__(self, other):
                return mycmp(self.obj, other.obj, ordem) < 0
    ...
    if 2 not in ordem:
        TAB.sort(key=cmp_to_key(reverse_numeric))
        return TAB
    else:
        a = transforma_data(TAB)
        TAB.sort(key=cmp_to_key(reverse_numeric))
        return transforma_data(TAB)
...
def main():
    ...
    while True:
        ...
        while True:
            ...
            start = time.process_time()
            classifica = CLASSIFICA(lista_sort, lista_ordem_int)
            ...
            for i in range (len(lista_separada)):
                for j in lista_ordem_int:
                    if b_bubble[i][j] != b_sort[i][j]:
                        print ("bubble x sort", b_bubble[i])

            for i in range (len(lista_separada)):
                for j in lista_ordem_int:
                    if b_quick[i][j] != b_sort[i][j]:
                        print ("quick x sort", b_quick[i])
